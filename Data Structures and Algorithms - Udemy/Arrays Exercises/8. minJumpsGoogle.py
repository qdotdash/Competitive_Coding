...
def returnMaxJumps(A):
	...
	while(limitIndex!=0):
		limitIndex = returnMaxInRange(limitIndex, A)
		if(limitIndex==-2):
			return -1
		else:
			jumps += 1
...
def returnMaxInRange(limitIndex, A): #function that returns the next index within limitIndex with max value that can reach upto or beyond limitIndex
	minIndex = 1000000
	j = limitIndex-1
	while(j>=0):
		if(j + A[j] >= limitIndex):
